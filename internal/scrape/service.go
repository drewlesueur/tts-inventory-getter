...
func (s Service) fetchListHTML(ctx context.Context, pageURL string, site config.SiteConfig, strategy string, cookies map[string]string) (string, error) {
	...
	if strings.Contains(strings.ToLower(site.Discovery.Notes), "hornemotors") {
		if h, err := fetchHorneMotorsInventoryHTML(ctx, pageURL); err == nil {
			if s.Logger != nil {
				s.Logger.Info("list html source", zap.String("url", pageURL), zap.String("source", "go_hornemotors_api"), zap.Int("cardCount", countCards(h, site.ListPage.CardSelector)))
			}
			return h, nil
		} else if s.Logger != nil {
			s.Logger.Warn("Go Horne Motors API fetch failed; falling back", zap.String("url", pageURL), zap.Error(err))
		}
	}
